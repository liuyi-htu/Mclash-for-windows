...
默认配置仅监听本机并关闭局域网代理；系统代理模式会绕过本机和常见私有网段。
Mclash 启用系统代理前会保存当前登录用户原有的代理设置，并在停止代理或切换到
TUN 模式时恢复。
IPv6 默认关闭，局域网绕过默认开启；修改后会同步更新当前运行配置。
自动生成的 `default.yaml` 可以直接删除；删除后切换运行模式也不会再次自动创建，
如需恢复请重新导入配置。
